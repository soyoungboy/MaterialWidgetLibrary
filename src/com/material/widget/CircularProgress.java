...
/**
 * Created by IntelliJ IDEA. User: keith. Date: 14-10-10. Time: 14:46.
 */
public class CircularProgress extends View {
	...
	public CircularProgress(Context context, AttributeSet attrs,
		...
		mIndeterminate = attributes.getBoolean(
				...
				getResources().getBoolean(
						R.bool.circular_progress_indeterminate));
		...
		mDuration = attributes.getInteger(
				R.styleable.CircularProgress_circular_progress_duration,
				PROGRESS_DURATION);
	...
	private RectF getArcRectF() {
		if (arcRectF == null) {
			int size = Math.min(getWidth() - mBorderWidth * 2, getHeight()
					- mBorderWidth * 2);
			arcRectF = new RectF();
		...
		if (widthSpecMode == MeasureSpec.AT_MOST
			...
			case SMALL_SIZE:
				size = getResources().getDimensionPixelSize(
						R.dimen.circular_progress_small_size);
				break;
			case NORMAL_SIZE:
				size = getResources().getDimensionPixelSize(
						R.dimen.circular_progress_normal_size);
				break;
			case LARGE_SIZE:
				size = getResources().getDimensionPixelSize(
						R.dimen.circular_progress_large_size);
				break;
			}
